...
export async function generateMetadata({ params }: RankingPageProps): Promise<Metadata> {
  ...
  const page = getCityRankingPageModel(city, "best-for-families");
  const definition = getCityRankingDefinition("best-for-families");
  ...
  return cityCollectionMetadata(title, description, `/${page.cityData.cityRouteSlug}/best-for-families`);
}

export default async function BestForFamiliesPage({ params }: RankingPageProps) {
  const { city } = await params;
  const page = getCityRankingPageModel(city, "best-for-families");
  const definition = getCityRankingDefinition("best-for-families");
    ...
    <>
      <SchemaScript
        data={breadcrumbSchema([
          ...
          { name: title, url: absoluteUrl(`/${page.cityData.cityRouteSlug}/best-for-families`) }
        ])}
      />
      <SchemaScript
        data={itemListSchema(
